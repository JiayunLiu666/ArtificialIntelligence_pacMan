...
def sentence1() -> Expr:
    ...
    a_or_b = A | B
    not_a_ifAndOnlyIf_not_b_or_c = ~A % (~B | C)
    not_a_or_not_b_or_c = disjoin(~A, ~B, C)

    return conjoin(a_or_b, not_a_ifAndOnlyIf_not_b_or_c, not_a_or_not_b_or_c)
    "*** END YOUR CODE HERE ***"
...
def sentence2() -> Expr:
    ...
    c_ifAndOnlyIf_b_or_D = C % (B | D)
    a_imply_not_b_and_not_d = A >> (~B & ~D)
    not_b_and_not_c_imply_a = ~(B & ~C) >> A
    not_d_imply_c = ~D >> C

    return conjoin(c_ifAndOnlyIf_b_or_D, a_imply_not_b_and_not_d, not_b_and_not_c_imply_a, not_d_imply_c)
    "*** END YOUR CODE HERE ***"
...
def sentence3() -> Expr:
    ...
    rule1 = PacmanAlive_1 % ((PacmanAlive_0 & ~PacmanKilled_0) | (~PacmanAlive_0 & PacmanBorn_0))
    not_alive_and_born_0 = ~(PacmanAlive_0 & PacmanBorn_0)

    return conjoin(rule1, not_alive_and_born_0, PacmanBorn_0)
    "*** END YOUR CODE HERE ***"
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # util.raiseNotDefined()
    unsatisfiedCondition = findModel(premise & (~conclusion))

    return unsatisfiedCondition == False
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    # util.raiseNotDefined()  
    literalInTwos = []
    for expr1, expr2 in itertools.combinations(literals, 2):
        literalInTwo = to_cnf(~(expr1 & expr2))
        literalInTwos.append(literalInTwo)
    return conjoin(literalInTwos)
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for x,y in all_coords:
        pacphysics_sentences.append(conjoin(PropSymbolExpr(wall_str, x, y) >> (~PropSymbolExpr(pacman_str, x, y, time=t))))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50): 
        print(f"Checking possibilities at time {t}")

        KB.append(exactlyOne([PropSymbolExpr(pacman_str, x, y, time=t) for (x, y) in non_wall_coords]))

        if model := findModel(conjoin(KB + [PropSymbolExpr(pacman_str, xg, yg, time=t)])):
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50): 
        ...
        for (x, y) in food:
            ...
            pacman_here = PropSymbolExpr(pacman_str, x, y, time=t)
            KB.append((current_food & pacman_here) >> ~next_food)
            KB.append(~pacman_here >> (next_food % current_food))

        KB.append(exactlyOne([PropSymbolExpr(act, time=t) for act in actions]))

        for (x, y) in non_wall_coords:
            if t > 0:  # No need for movement axioms at t=0
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        "*** BEGIN YOUR CODE HERE ***"
        # util.raiseNotDefined()
        current_axioms = pacphysicsAxioms(t, all_coords, non_outer_wall_coords, walls_grid, sensorAxioms, allLegalSuccessorAxioms)
        KB.append(current_axioms)

        current_action_expr = PropSymbolExpr(agent.actions[t], t)
        KB.append(current_action_expr)
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for x, y in non_outer_wall_coords:
            wall_prop = PropSymbolExpr(wall_str, x, y)
            no_wall_prop = ~wall_prop
            if entails(conjoin(KB), wall_prop):
                if known_map[x][y] != 1:
                    KB.append(wall_prop)
                    known_map[x][y] = 1
            elif entails(conjoin(KB), no_wall_prop):
                if known_map[x][y] != 0:
                    KB.append(no_wall_prop)
                    known_map[x][y] = 0
...
def slam(problem, agent) -> Generator:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # util.raiseNotDefined()
    initial_pacman_state = PropSymbolExpr(pacman_str, pac_x_0, pac_y_0, time=0)
    KB.append(initial_pacman_state)

    no_wall_at_initial = ~PropSymbolExpr(wall_str, pac_x_0, pac_y_0)
    KB.append(no_wall_at_initial)
    ...
    for t in range(agent.num_timesteps):
        ...
        "*** BEGIN YOUR CODE HERE ***"
        # – Addpacphysics,action,andperceptinformationtoKB.UseSLAMSensorAxioms,SLAMSuccessorAxioms, and numAdjWallsPerceptRules.
