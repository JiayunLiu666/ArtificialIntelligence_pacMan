...
class DiscreteDistribution(dict):
    ...
    def normalize(self):
        ...
        # raiseNotDefined()
        # use total method to find the sum
        # empty distribution / values are zero
        sum = self.total()
        if sum != 0:
            for i in self.keys():
                self[i] = 1 / sum * self[i]
    ...
    def sample(self):
        ...
        self.normalize()
        sum = 0
        randomSample = 1.0 * random.random()
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        "*** BEGIN YOUR CODE HERE ***"
        # raiseNotDefined()
        # iterate your updates over the variable self.allPositions which includes all legal positions plus the special jail position
       ...
       # update the belief at every position on the map after one time step elapsing
        ...
        for oldPos in self.allPositions:
            ...
            for newPos, newPosProb in newPosDist.items():
                updatedBeliefs[newPos] += newPosProb * oldPosProb
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** BEGIN YOUR CODE HERE ***"
        # raiseNotDefined()
        repeats, extra = divmod(self.numParticles, len(self.legalPositions))

        for pos in self.legalPositions:
            self.particles.extend([pos] * repeats)

        self.particles.extend(self.legalPositions[:extra])
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            observationProb = self.getObservationProb(observation, pacPosition, particle, jailPosition)
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            newPosDistribution = self.getPositionDistribution(gameState, oldParticle)
            updatedParticles.append(newPosDistribution.sample())
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        while len(self.particles) < self.numParticles:
            self.particles.append(permutations[index % len(permutations)])
            index += 1
        ...
        for particle in self.particles:
            ...
            for ghostIndex in range(self.numGhosts):
                ...
                obsProb = self.getObservationProb(
                    observation[ghostIndex], pacmanPosition, ghostPosition, jailPosition)
                particleProbability *= obsProb
            weights[particle] += particleProbability
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            # Update each ghost's position in the particle based on the transition model
