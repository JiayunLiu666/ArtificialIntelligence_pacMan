...
def joinFactors(factors: ValuesView[Factor]):
    ...
    # raiseNotDefined()
    # STEP1. find out unconditioned or conditioned
    unconditioned = set()
    conditioned = set()
    ...
    resVar = conditioned - unconditioned

    variableDomainsDict = next(iter(factors)).variableDomainsDict()

    # STEP2. create a new factor 
    newFactor = Factor(unconditioned, resVar, variableDomainsDict)
...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor: Factor, eliminationVariable: str):
        ...
        newConditioned = set(factor.conditionedVariables()) - {eliminationVariable}
        ...
        for assignment in newFactor.getAllPossibleAssignmentDicts():
            prob = 0
            for elimated in factor.variableDomainsDict()[eliminationVariable]:
                assignment[eliminationVariable] = elimated
                prob += factor.getProbability(assignment)
            newFactor.setProbability(assignment, prob)
