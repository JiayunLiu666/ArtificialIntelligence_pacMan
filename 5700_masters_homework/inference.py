...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # raiseNotDefined()
    # STEP1. add variables and edges
    variables = [GHOST0, OBS0, PAC, OBS1, GHOST1]
    edges = [(GHOST0, OBS0), (PAC, OBS0), (PAC, OBS1), (GHOST1, OBS1)]

    # STEP2. position tuples for Pacman and 2 ghosts
    # get all the positins in the grid
    position = []
    ...
    # STEP3. observed distance is a noisy Manhattan distance: it's non-negative and |obs - true| <= MAX_NOISE
    maxManhattanDist = X_RANGE - 1 + Y_RANGE - 1
    observedDist = maxManhattanDist + MAX_NOISE + 1

    observation = []
    for obs in range(observedDist):
        observation.append(obs)

    # STEP4. set each `variableDomainsDict[var] = values`
    variableDomainsDict[PAC] = position
    variableDomainsDict[GHOST0] = position
    variableDomainsDict[GHOST1] = position
    variableDomainsDict[OBS0] = observation
    variableDomainsDict[OBS1] = observation
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for eliminated in eliminationOrder:
            factorsNotJoined, factorsJoined = joinFactorsByVariable(factors, eliminated)

            #  take care of the special case where a factor you have joined only has one unconditioned variable 
            if len(factorsJoined.unconditionedVariables()) != 1:
                newFactor = eliminate(factorsJoined, eliminated)
                factorsNotJoined.append(newFactor)

            factors = factorsNotJoined

        ansFactor = joinFactors(factors)

        return normalize(ansFactor)
        "*** END YOUR CODE HERE ***"
